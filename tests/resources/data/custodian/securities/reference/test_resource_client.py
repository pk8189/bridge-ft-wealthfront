...
import typing
from wealthtech_api.types.data.custodian.securities.reference import models
...
def test_create_200_generated_success_default():
    ...
    response = client.data.custodian.securities.reference.create(
        data={"reported_date": "1970-01-01"}
    )
    adapter = TypeAdapter(
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.data.custodian.securities.reference.create(
        data={"reported_date": "1970-01-01"}
    )
    adapter = TypeAdapter(
        typing.List[models.PostDataCustodianSecuritiesReferenceResponseItem]
