...
import typing
from wealthtech_api.types.data.custodian.securities.fetch import models
...
def test_create_200_generated_success_default():
    ...
    response = client.data.custodian.securities.fetch.create(
        pager_limit=123, pager_page=123, data={"ids": [123]}
    )
    adapter = TypeAdapter(typing.List[models.Security])
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.data.custodian.securities.fetch.create(
        pager_limit=123, pager_page=123, data={"ids": [123]}
    )
    adapter = TypeAdapter(typing.List[models.Security])
    adapter.validate_python(response)
