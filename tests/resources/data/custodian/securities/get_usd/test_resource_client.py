...
def test_list_200_generated_success():
    ...
    response = client.data.custodian.securities.get_usd.list(
        pager_limit=123, pager_page=123
    )
    adapter = TypeAdapter(models.GetDataCustodianSecuritiesGetUsdResponse)
...
async def test_await_list_200_generated_success():
    ...
    response = await client.data.custodian.securities.get_usd.list(
        pager_limit=123, pager_page=123
    )
    adapter = TypeAdapter(models.GetDataCustodianSecuritiesGetUsdResponse)
    adapter.validate_python(response)
