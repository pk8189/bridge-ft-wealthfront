...
def test_create_200_generated_success():
    ...
    response = client.billing.groups.create_from_households.create(
        pager_limit=123, pager_page=123
    )
    adapter = TypeAdapter(models.PostBillingGroupsCreateFromHouseholdsResponse)
...
async def test_await_create_200_generated_success():
    ...
    response = await client.billing.groups.create_from_households.create(
        pager_limit=123, pager_page=123
    )
    adapter = TypeAdapter(models.PostBillingGroupsCreateFromHouseholdsResponse)
    adapter.validate_python(response)
