...
from wealthtech_api.types.billing.groups.delete_many import models
...
def test_create_200_generated_success_default():
    ...
    response = client.billing.groups.delete_many.create(
        pager_limit=123, pager_page=123, data={"ids": [123]}
    )
    adapter = TypeAdapter(models.PostBillingGroupsDeleteManyResponse)
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.billing.groups.delete_many.create(
        pager_limit=123, pager_page=123, data={"ids": [123]}
    )
    adapter = TypeAdapter(models.PostBillingGroupsDeleteManyResponse)
    adapter.validate_python(response)
