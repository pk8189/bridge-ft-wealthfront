...
from wealthtech_api.types.reporting.class_tags.create_many import models
...
def test_create_200_generated_success_default():
    ...
    response = client.reporting.class_tags.create_many.create(
        pager_limit=123, pager_page=123, data=[{"firm_id": 39, "name": "International"}]
    )
    adapter = TypeAdapter(models.PostReportingClassTagsCreateManyResponse)
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.reporting.class_tags.create_many.create(
        pager_limit=123, pager_page=123, data=[{"firm_id": 39, "name": "International"}]
    )
    adapter = TypeAdapter(models.PostReportingClassTagsCreateManyResponse)
    adapter.validate_python(response)
