...
from wealthtech_api.types.reporting.class_tags.delete_many import models
...
def test_create_200_generated_success_default():
    ...
    response = client.reporting.class_tags.delete_many.create(
        pager_limit=123, pager_page=123, data={"ids": [123]}
    )
    adapter = TypeAdapter(models.PostReportingClassTagsDeleteManyResponse)
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.reporting.class_tags.delete_many.create(
        pager_limit=123, pager_page=123, data={"ids": [123]}
    )
    adapter = TypeAdapter(models.PostReportingClassTagsDeleteManyResponse)
    adapter.validate_python(response)
