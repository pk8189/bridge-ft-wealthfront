...
from wealthtech_api.types.reporting.benchmarks.create_many import models
...
def test_create_200_generated_success_default():
    ...
    response = client.reporting.benchmarks.create_many.create(
        pager_limit=123,
        pager_page=123,
        data=[{"firm_id": 39, "name": "Major Index", "slug": "major-index"}],
    )
    adapter = TypeAdapter(models.PostReportingBenchmarksCreateManyResponse)
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.reporting.benchmarks.create_many.create(
        pager_limit=123,
        pager_page=123,
        data=[{"firm_id": 39, "name": "Major Index", "slug": "major-index"}],
    )
