...
from wealthtech_api.types.reporting.target_allocations.filter import models
...
def test_create_200_generated_success_default():
    ...
    response = client.reporting.target_allocations.filter.create(
        pager_limit=123,
        pager_page=123,
        data={"firm_id": 39, "id": 1, "name": "Conservative", "slug": "conservative"},
    )
    adapter = TypeAdapter(models.PostReportingTargetAllocationsFilterResponse)
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.reporting.target_allocations.filter.create(
        pager_limit=123,
        pager_page=123,
        data={"firm_id": 39, "id": 1, "name": "Conservative", "slug": "conservative"},
    )
