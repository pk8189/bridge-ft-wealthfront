...
from wealthtech_api.types.reporting.target_allocations.delete_many import models
...
def test_create_200_generated_success_default():
    ...
    response = client.reporting.target_allocations.delete_many.create(
        pager_limit=123, pager_page=123, data={"ids": [123]}
    )
    adapter = TypeAdapter(models.PostReportingTargetAllocationsDeleteManyResponse)
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.reporting.target_allocations.delete_many.create(
        pager_limit=123, pager_page=123, data={"ids": [123]}
    )
    adapter = TypeAdapter(models.PostReportingTargetAllocationsDeleteManyResponse)
    adapter.validate_python(response)
