...
import typing
from wealthtech_api.types.org.advisor_codes.create_many import models
...
def test_create_200_generated_success_default():
    ...
    response = client.org.advisor_codes.create_many.create(
        data=[{"code_field": "NEWCODE", "firm_id": 39, "source": "APX"}]
    )
    adapter = TypeAdapter(typing.List[models.PostOrgAdvisorCodesCreateManyResponseItem])
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.org.advisor_codes.create_many.create(
        data=[{"code_field": "NEWCODE", "firm_id": 39, "source": "APX"}]
    )
    adapter = TypeAdapter(typing.List[models.PostOrgAdvisorCodesCreateManyResponseItem])
    adapter.validate_python(response)
