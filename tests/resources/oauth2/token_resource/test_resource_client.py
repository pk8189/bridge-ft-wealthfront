...
from wealthtech_api.types.oauth2.token_resource import models
...
def test_create_200_generated_success_default():
    ...
    response = client.oauth2.token_resource.create(
        data={"grant_type": "client_credentials"}
    )
    adapter = TypeAdapter(models.Oauth)
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.oauth2.token_resource.create(
        data={"grant_type": "client_credentials"}
    )
    adapter = TypeAdapter(models.Oauth)
    adapter.validate_python(response)
