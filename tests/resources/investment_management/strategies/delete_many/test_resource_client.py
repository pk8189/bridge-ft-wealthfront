...
from wealthtech_api.types.investment_management.strategies.delete_many import models
...
def test_create_200_generated_success_default():
    ...
    response = client.investment_management.strategies.delete_many.create(
        pager_limit=123, pager_page=123, data={"ids": [123]}
    )
    adapter = TypeAdapter(models.PostInvestmentManagementStrategiesDeleteManyResponse)
...
async def test_await_create_200_generated_success_default():
    ...
    response = await client.investment_management.strategies.delete_many.create(
        pager_limit=123, pager_page=123, data={"ids": [123]}
    )
    adapter = TypeAdapter(models.PostInvestmentManagementStrategiesDeleteManyResponse)
    adapter.validate_python(response)
