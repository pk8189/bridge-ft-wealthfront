...
def test_list_200_generated_success():
    ...
    response = client.account_management.tda_to_swb_migrations.list(
        pager_limit=123, pager_page=123
    )
    adapter = TypeAdapter(models.GetAccountManagementTdaToSwbMigrationsResponse)
...
async def test_await_list_200_generated_success():
    ...
    response = await client.account_management.tda_to_swb_migrations.list(
        pager_limit=123, pager_page=123
    )
    adapter = TypeAdapter(models.GetAccountManagementTdaToSwbMigrationsResponse)
    adapter.validate_python(response)
